...
Предложения, замечания, идеи, вопросы по установке и сообщения об ошибках можно оставлять в GitHub Discussions:

[Предложения, замечания и идеи по RamTeamAi](https://github.com/korobprog/RamTeamAi/discussions/3)
...
- сервис точечных правок после сборки: live-preview, DOM-инспектор, чат правок, терминал, логи и карта проекта в одном полноэкранном workbench;
...
| --- | --- | --- | --- |
| `0.1.5` | 2026-06-20 | Fixed post-update GitHub profile status: local GitHub authorization now stays green when Firebase cloud sync is not configured; added avatar fallback for blocked GitHub images. | Windows: [AppSetup.exe](https://github.com/korobprog/RamTeamAi/releases/download/v0.1.5/AppSetup.exe) / [NSIS installer](https://github.com/korobprog/RamTeamAi/releases/download/v0.1.5/RamTeamAi_0.1.5_x64-setup.exe); macOS: [DMG](https://github.com/korobprog/RamTeamAi/releases/download/v0.1.5/RamTeamAi_0.1.5_universal.dmg); updater: [latest.json](https://github.com/korobprog/RamTeamAi/releases/download/v0.1.5/latest.json) |
...
- Подготовлен новый релиз 0.1.5: синхронизированы версии проекта и заметки к сборке.
- Версию синхронизируем командой `npm run version:set -- 0.1.5`.
...
### 10. Сервис точечных правок после сборки

![Сервис точечных правок RamTeamAi](assets/screenshots/08-post-build-workbench.png)

Экран **Правки** превращает готовый build-артефакт в рабочую среду для точечных изменений. Слева находится чат правок и выбранный DOM-элемент: можно отправить конкретный блок интерфейса агенту как контекст, не объясняя заново всю страницу. В центре расположен live-preview в браузерной рамке с подсветкой выбранной области и быстрыми карточками инспектора. Снизу видны команды запуска, readiness-статус, терминал и события файлов, а справа — карта проекта со стеком, маршрутами, компонентами, backend/IPC, потоками данных и рисками.

Такой сервис нужен, чтобы после генерации проекта быстро доводить интерфейс до нужного вида: выбрать точный элемент, описать изменение, проверить результат в preview и сразу видеть, какие файлы и проверки затронуты.
...
For public GitHub Actions releases, `VITE_GITHUB_CLIENT_ID` must be added to repository secrets
and passed through `.github/workflows/release.yml`. Firebase `VITE_FIREBASE_*` values are optional:
if they are missing, release builds continue and cloud sync stays disabled until Firebase is configured.
