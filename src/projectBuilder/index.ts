...
export function planImplementationAssignments(
  ...
  const summaries: Record<string, { summary: string; deliverables: string[] }> = {
    ...
    tester: {
      summary: "Финально запускает проект как QA: зависимости, build/lint/test/check, браузерные сценарии, Browser/Playwright MCP и DevTools.",
      deliverables: ["Adaptive app-specific test checklist", "Stack-matched automated test files and test command updates", "Чеклист запуска и автотестов", "Browser/MCP smoke-сценарии", "Демо-аккаунт для авторизации при необходимости", "Чеклист ошибок для возврата агентам"],
    },
    security: {
...
export function renderImplementationPlan(artifact: PlanArtifact, assignments: ImplementationAssignment[]): string {
  ...
  const lead = assignments.find((item) => item.role === "architect")
    ?? assignments.find((item) => item.role === "arbiter")
    ?? assignments[0];
...
- После каждого блока обновлять этот файл, \`PLAN.md\` и \`docs/agent-tasks.md\`.

## Самоуправляемый чеклист агентов

- Главный агент: ${lead ? `${lead.owner} (${lead.role})` : "первый доступный агент"}.
- Каждый агент сам добавляет свои подзадачи в этот файл, берёт следующий незакрытый пункт и помечает его как \`В работе: @имя\`.
- После записи файлов агент переводит пункт в \`[x]\` и добавляет короткое доказательство: какие файлы изменены.
- Если проверка или автотест падает, QA-агент не останавливает процесс: он добавляет ниже чеклист правок и возвращает его разработчикам в следующем раунде.

## Следующие задачи агентам

${taskList}

## Тестовый 3-step сценарий QA-агента

QA-агент создаёт и поддерживает этот раздел сам; это не ручной чеклист пользователя. Если QA-бот есть в команде, он может добавлять проверки под конкретный тип приложения, автотесты, тестовые данные и команды запуска.

1. [ ] Подготовить окружение: зависимости, env-пример, команды запуска.
2. [ ] Проверить автоматически: build/lint/test/check и stack-matched автотесты.
3. [ ] Проверить пользовательский сценарий: UI через Browser/Playwright MCP/DevTools, демо-аккаунт для авторизации при необходимости, затем вернуть ошибки разработчикам или отметить готовность.

## Проверки

- [ ] Установить зависимости проекта (\`npm install\` / pnpm / yarn / bun) перед запуском проверок.
- [ ] Запустить доступные команды: dev/build/lint/test/check и сохранить результаты.
- [ ] Открыть проект в браузере и проверить ключевые сценарии через Browser/Playwright MCP и DevTools.
- [ ] Если в проекте есть авторизация — создать демо-аккаунт и пройти пользовательский путь от входа до целевого действия.
- [ ] Если проверка падает — оформить чеклист ошибок, вернуть его агентам на правку и не уведомлять о готовности.
- [ ] Если все проверки прошли — уведомить пользователя, что проект готов.
`;
}
