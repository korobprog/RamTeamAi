...
const IMPLEMENTATION_ARTIFACT_INSTRUCTIONS = [
  "РЕЖИМ РЕАЛИЗАЦИИ. Ты пишешь рабочий код прямо сейчас, в этом сообщении.",
  "ЗАПРЕЩЕНО: отвечать только планом или списком файлов; писать «создам», «подключим», «в следующей итерации», «нужно сделать», «далее»; описывать словами, что файл будет содержать.",
  "Возьми 1-3 файла из плана и выдай их ПОЛНОЕ содержимое в этом же ответе.",
  "Формат строго такой: отдельная строка `Файл: путь/к/файлу`, а сразу под ней fenced code block с полным кодом файла. Только так приложение запишет файл на диск.",
  ...
  "}",
  "```",
  "Можно добавить 1-2 строки пояснения, но код в блоках обязателен. Ответ без хотя бы одного `Файл:` + fenced code block считается невыполненным заданием.",
].join("\n");
